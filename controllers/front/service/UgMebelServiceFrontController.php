...
class UgMebelServiceFrontController extends \controllers\base\Controller
{
	...
	protected function sitemap()
	{
		...
		if ($contents === false){
			...
			$articles->setSubquery($query, \modules\articles\lib\ArticleConfig::ACTIVE_STATUS_ID, '119');
			...
			$goods->setSubquery(' AND `statusId` NOT IN (?d, ?d) AND `fabricatorId` = ?d',
					...
					$this->getController('Catalog')->getUgFabricatorId());
    ...
    protected function yml()
    {
        ...
        $yml->setShopName('Юг мебель')
            ->setCompanyName('Юг мебель')
            ->addCategories($this->getController('Catalog')->getCategoriesByFabricatorId( $this->getController('Catalog')->getUgFabricatorId() ));

        foreach($this->getController('Catalog')->getItemsByFabricatorId( $this->getController('Catalog')->getUgFabricatorId() ) as $good)
