...
class MeriMebelServiceFrontController extends \controllers\base\Controller
{
	...
	protected function sitemap()
	{
		...
		if ($contents === false){
			...
			$articles->setSubquery($query, \modules\articles\lib\ArticleConfig::ACTIVE_STATUS_ID, '115');
			...
			$goods->setSubquery(' AND `statusId` NOT IN (?d, ?d) AND `fabricatorId` = ?d',
					...
					$this->getController('Catalog')->getMeriFabricatorId());
    ...
    protected function yml()
    {
        ...
        $yml->setShopName('Мери мебель')
            ->setCompanyName('Мери мебель')
            ->addCategories($this->getController('Catalog')->getCategoriesByFabricatorId( $this->getController('Catalog')->getMeriFabricatorId() ));

        foreach($this->getController('Catalog')->getItemsByFabricatorId( $this->getController('Catalog')->getMeriFabricatorId() ) as $good)
