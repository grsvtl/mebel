...
class OffersAdminController extends \controllers\base\Controller
{
	...
	protected $permissibleActions = array(
		...
		'ajaxAddOfferGroupGood',
		'removeOfferGroup',
		'editOfferGroup',

		'getGroupGoodsTable',
		'addOfferGroupGood',
		'removeOfferGroupGood',
	...
	protected function getGroupGoodsTable($offer)
	{
		echo $this->getGroupGoodsTableContent($offer);
	}

	private function getGroupGoodsTableContent(\modules\catalog\offers\lib\Offer $offer)
	{
		$goods = $this->getOfferGroupGoods()->getGoodsByOffer($offer);

		$config = $this->getObject('\modules\catalog\offers\offerGroups\offerGroupGoods\lib\OfferGroupGoodConfig');
		ob_start();
			$this->setContent('goods', $goods)
				->includeTemplate($config->getAdminTemplateDir().'groupGoodsTable');
			$contents = ob_get_contents();
		ob_end_clean();
		return $contents;
	}

	private function getOfferGroups()
	{
		return new \modules\catalog\offers\offerGroups\lib\OfferGroups;
	}

	private function getOfferGroupGoods()
	{
		return new \modules\catalog\offers\offerGroups\offerGroupGoods\lib\OfferGroupGoods;
	}

	protected function ajaxAddOfferGroupGood()
	{
		$data = $this->getPOST();
		$offerGroup = $this->getOfferGroups()->getOfferGroupByOfferId($data->offerId);

		if(!$offerGroup)
			$offerGroup = $this->getOfferGroups()->getObjectById( $this->getOfferGroups()->add(array('offerId'=>$data->offerId)) );

		$res = $offerGroup->addGood( $data->goodId);
		$offer = $this->getObject($this->objectClass, $data->offerId);

		$this->ajaxResponse( is_int($res) ? $this->getGroupGoodsTableContent($offer) : $res);
	}

	protected function removeOfferGroupGood()
	{
		$offerGroupGood = $this->getObject( '\modules\catalog\offers\offerGroups\offerGroupGoods\lib\OfferGroupGood', $this->getPOST()['removedGoodId'] );
		$goodId = $this->getObject('\modules\catalog\offers\offerGroups\offerGroupGoods\lib\OfferGroupGood', $this->getPOST()['removedGoodId'])->goodId;
		$offer = $this->getObject($this->objectsClass)->getOfferFromGroupsByGoodId($goodId, 'all');
		$res = $offerGroupGood->remove();
		$this->ajaxResponse( is_int($res) ? $this->getGroupGoodsTableContent($offer) : $res);
	}
}