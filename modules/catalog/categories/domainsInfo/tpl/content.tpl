					...
					<form class="formEdit" action="/admin/domainsInfo/editDomainCategoryInfo/" method="post" data-post-action="none">
						<input type="hidden" name="id" value="<?=$category->id?>" id="objectId"/>
...
<?if ($object->id):?>
											<tr>
												<td class="first" colspan="2" style="text-align: center;">
													<a href="<?=$category->getPathByDomainAlias($domainAlias);?>" title="Открыть объект в клиентской части" target="_blank">
														Просмотреть объект
													</a>
												</td>
											</tr>
