				...
				<div class="table_edit">
					...
					<table  id="objects-tbl" data-sortUrlAction="/admin/offers/changePriority/?" width="100%">
						...
						<? $i=1; foreach ($objects as $object):?>
							<tr id="id<?=$object->id?>" class="dblclick <?= $object->isValid() ? '' : 'lightGray'?>"
								data-url="/admin/offers/offer/<?= $object->id?>" data-id="<?= $object->id?>"
								data-priority="<?= $object->priority?>"
							>
								<td><?$object->isValid()?><input type="checkbox" class="groupElements" /></td>
								<td><?=$i?></td>
								<td><?=$object->id?></td>
								<td>
									<p><?=$object->name?></p>
									<?$offerGoods = (new \modules\catalog\offers\offerGroups\offerGroupGoods\lib\OfferGroupGoods())->getGoodsByOffer($object)?>
									<div class="simpleTooltip">
										<?= $offerGoods->count()
											?
											' <span class="blue">'.$offerGoods->count()
												.' '.\core\utils\Utils::declension($offerGoods->count(), array('товар','товара','товаров'))
												.'</span>'
											:
											''?>
										<?if($offerGoods->count()):?>
										<span class="simpleTooltipShow">
											<?foreach($offerGoods as $offerGood):?>
												<?=$offerGood->getGood()->getName()?>
												<br />
											<?endforeach?>
										</span>
										<?endif?>
								...
								<td><p><?=$object->getTypeName()?></p></td>
								<td><p><?=$object->getTypeRemain()?></p></td>
