...
class OrderConfig extends \core\modules\base\ModuleConfig
{
	...
	public function rules()
	{
		return array(
			...
			'partnerNotified, paidPercent' => array(
				'adapt' => '_adaptBool',
			),
			'clientId' => array(
				'validation' => array('_validInt', array('notEmpty'=>true)),
			),
			'country, city, street, home, phone, domain' => array(
	...
	public function _adaptNr($key)
	{
		...
		if(empty($this->data[$key])){
			$id = empty($this->data['id']) ? $orders->getNextId() : $this->data['id'];
			$this->data[$key] = $id.'-'.date("d-m-y");
		}
	}
