...
<tr>
	...
	<td style="border-bottom: 1px dotted #007fb9;">
		<a href="http://<?= $_SERVER['HTTP_HOST']?><?=$primaryGood->getPath()?>" target="_blank">
			...
			<? else: ?>
			<img border="0" src="http://<?= $_SERVER['HTTP_HOST']?><?=$primaryGood->getGood()->getFirstPrimaryImage()->getImage('50x50')?>" height="50" width="50">
			<? endif; ?>
		</a>
	</td>
		...
		<table style="width: 100%;">
			<tr>
				<td valign="top">
					...
					<ul>
						<li><a href="http://<?= $_SERVER['HTTP_HOST']?><?=$primaryGood->getGood()->getPath()?>" target="_blank"><?=$primaryGood->getGood()->getName()?> (<?=$primaryGood->getGood()->getCode()?>)</a></li>
						<?foreach($secondaryGoods as $secondaryGood):?>
						<li><a href="http://<?= $_SERVER['HTTP_HOST']?><?=$secondaryGood->getGood()->getPath()?>" target="_blank"><?=$secondaryGood->getGood()->getName()?> (<?=$secondaryGood->getGood()->getCode()?>)</a></li>
						<?endforeach?>
					</ul>
				</td>
			</tr>
