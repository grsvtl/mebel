			...
			success: function(data){
				...
				if(data){
					if( $.isNumeric(data) ){
						...
						$((new shopcartHandler()).sources.shopcartContent).html('Спасибо. Ваш заказ принят. Мы перезвоним Вам в ближайшее время.');
                                                
                                                $('.yandex_s_p form input[type="submit"]').live('click', function(){
                                                    dataLayer.push({'event': 'event_zakaz_paynow'});
                                                    console.log('event_zakaz_paynow');
                                                });
