...
abstract class Model
{
	...
	public function getMinField($field, $table = null)
	{
		if (empty($field))
			throw new \Exception('Field name is empty in '.get_class($this).'!');
		$table = (empty ($table)) ? $this->mainTable : $table;
		$query = 'SELECT MIN(`?s`) as `res` FROM `?s`';
		$data = array ($field, $table);
		$result = db\Db::getMysql()->row($query, $data);
		return (is_numeric($result['res'])) ? $result['res'] : false;
	}

	public function getMaxField($field, $table = null)
	{
		if (empty($field))
			throw new \Exception('Field name is empty in '.get_class($this).'!');
		$table = (empty ($table)) ? $this->mainTable : $table;
		$query = 'SELECT MAX(`?s`) as `res` FROM `?s`';
		$data = array ($field, $table);
		$result = db\Db::getMysql()->row($query, $data);
		return (is_numeric($result['res'])) ? $result['res'] : false;
	}
