...
// log sql on end
	...
	{
		$q = 'SELECT COUNT(*) as quantity FROM ( SELECT * FROM '.$q_count.' ) as newTable';
		$row = $this->row($q,$data);
		return $row['quantity'];
	}
	...
	static public function getSubqueryStringResult($query, $data = array())
	{
		if(!is_array($data))
			$data = array($data);

		$value = Db::getMysql()->rows($query, $data);
		$result = [];
		if (is_array($value))
